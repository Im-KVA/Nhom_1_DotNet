...
namespace Nhom_1_DotNet
{
    public partial class FrmLogin : Form
    {
        ...
        private void PerformLogin()
        {
            ...
            if (ValidateUser(username, password, selectedRole))
            {
                ...
                {
                    case "SINHVIEN":
                        new FrmSinhVien(username).Show();
                        break;
                    case "GIANGVIEN":
                        new FrmGVHD(username).Show();
                        break;
                    case "ADMIN":
        ...
        private bool ValidateUser(string username, string password, string role)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                if (reader.Read())
                {
                    username = role == "SINHVIEN" ? reader["MASV"].ToString() : reader["MAGIANGVIEN"].ToString(); // Lưu mã tương ứng
